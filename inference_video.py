...
def main(args):
    ...
    # Path video input
    video_path = "/content/drive/MyDrive/testing_slow/test1_normal.mp4"

    # Membaca video menggunakan OpenCV
    cap = cv2.VideoCapture(video_path)

    # Inisialisasi video output
    output_path = "/content/drive/MyDrive/testing_slow/normal/test1_normal_count2.mp4"
    output_folder = "/content/drive/MyDrive/testing_slow/normal/predict2"
    ...
    while cap.isOpened():
        ...
        if len(boxes[0]) > 0:
            ...
            for xmin, ymin, xmax, ymax in boxes[0].tolist():
                ...
                if ymin - 18 >= 0:
                    ymin -= 18
                draw.text((xmin, ymin), f"Crack: {scores[0][0]:.4f}", fill="green", font=font, spacing=0.2)
